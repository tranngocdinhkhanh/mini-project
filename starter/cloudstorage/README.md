...
Super*Duper*Drive wants you to focus on building the web application with the skills you acquired in this course. That means you are responsible for developing the server, website, but other tasks like deployment belong to other teams at the company. 
...
3. Making calls to the database with JPA
 ...
 - To connect these model classes with database data, implement JPA interfaces for each of the model types. These classes should have methods that represent specific SQL queries and statements required by the functionality of the application. They should support the basic CRUD (Create, Read, Update, Delete) operations for their respective models at the very least. You can place these classes in (you guessed it!) the `repository` package.
